...
from doctr.datasets import DetectionDataset, SynthText, FUNSD
...
def main(args):
    ...
    if not isinstance(args.workers, int):
        args.workers = min(8, mp.cpu_count())

    torch.backends.cudnn.benchmark = True

    st = time.time()
    val_set = FUNSD(train=False, download=True,
    ...
    train_set = SynthText(
        train = True, predownload_path = args.train_path,
